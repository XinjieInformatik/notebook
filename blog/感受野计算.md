...
<img src="http://latex.codecogs.com/gif.latex?$$&space;padding&space;=&space;\frac{(shape_{out}&space;-&space;1)&space;*&space;stride&space;&plus;&space;filter_{size}&space;-&space;shape_{in}}{2}&space;$$" title="$$ padding = \frac{(shape_{out} - 1) * stride + filter_{size} - shape_{in}}{2} $$" />

- 如果特征图大小不变
- 
$$ padding = \frac{filter_{size} - 1}{2} $$

<img src="http://latex.codecogs.com/gif.latex?$$&space;padding&space;=&space;\frac{filter_{size}&space;-&space;1}{2}&space;$$" title="$$ padding = \frac{filter_{size} - 1}{2} $$" />
